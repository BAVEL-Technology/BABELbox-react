  ...
  return (
    <div className="card my-6 flex items-center justify-between bg-green-400 w-full text-gray-700 p-4 text-xl lg:text-4xl md:text-3xl text-center rounded-xl tracking-widest shadow-lg hover:shadow-xl transform hover:-translate-y-2" style={{fontFamily: "'Sniglet', cursive"}}>
      <div onClick={() => {changeEmoji();}} className="emoji cursor-pointer rounded-full hover:bg-blue-300 p-2">{props.user.avatar}</div>
        <input onkeyup={()=> {changeUserName();}} type="text" id="user-name-change" value={props.user.name} className="appearance-none text-center bg-green-400 focus:outline-none" />
